  ...
  _createClass(Wizard, [{
    ...
    value: function attachNav() {
      ...
      _lodash.default.each(this.buttons, function (button) {
        var buttonElement = _this7.refs["".concat(_this7.wizardKey, "-").concat(button.name)];

        _this7.addEventListener(buttonElement, 'click', function (event) {
          event.preventDefault(); // Disable the button until done.

          buttonElement.setAttribute('disabled', 'disabled');

          _this7.setLoading(buttonElement, true); // Call the button method, then re-enable the button.


          _this7[button.method]().then(function () {
            buttonElement.removeAttribute('disabled');

            _this7.setLoading(buttonElement, false);
          }).catch(function () {
            buttonElement.removeAttribute('disabled');
    ...
    value: function attachHeader() {
      ...
      if (this.isBreadcrumbClickable()) {
        this.refs["".concat(this.wizardKey, "-link")].forEach(function (link, index) {
          _this8.addEventListener(link, 'click', function (event) {
            _this8.emit('wizardNavigationClicked', _this8.pages[index]);

            event.preventDefault();
            return _this8.setPage(index).then(function () {
              _this8.emit('wizardPageSelected', _this8.pages[index], index);
            });
          });
        });
      }
    ...
    value: function detachNav() {
      ...
      _lodash.default.each(this.buttons, function (button) {
        _this9.removeEventListener(_this9.refs["".concat(_this9.wizardKey, "-").concat(button.name)], 'click');
      });
    }
  }, {
    key: "detachHeader",
    value: function detachHeader() {
      ...
      this.refs["".concat(this.wizardKey, "-link")].forEach(function (link) {
        _this10.removeEventListener(link, 'click');
      });
    }
  }, {
    key: "transformPages",
    value: function transformPages() {
      ...
      (_this$originalCompone = this.originalComponents) === null || _this$originalCompone === void 0 ? void 0 : _this$originalCompone.forEach(function (item) {
        if (!item.key) {
          item.key = item.title;
        }
      ...
      var getAllComponents = function getAllComponents(nestedComp, compsArr) {
        ...
        var hasNested = false;
        var nestedPages = [];
        var components = (nestedComp === null || nestedComp === void 0 ? void 0 : nestedComp.subForm) ? nestedComp === null || nestedComp === void 0 ? void 0 : nestedComp.subForm.components : (nestedComp === null || nestedComp === void 0 ? void 0 : nestedComp.components) || [];
        var additionalComponents = components.filter(function (comp) {
          return !comp.subForm && comp._visible;
        });
        (0, _utils.eachComponent)(components, function (comp) {
      ...
      if (this.originalComponents) {
        this.originalComponents.forEach(function (item) {
          if (item.type === 'panel') {
            ...
            var isVisible = (0, _utils.checkCondition)(item, data, data, _this11.component, _this11);

            if (isVisible) {
              visible.push(item);

              if (page) {
                _this11.pages.push(page);
              }
            }

            if (!page && isVisible) {
              page = _this11.createComponent(item, pageOptions);
              page.visible = isVisible;

              _this11.pages.push(page);

              page.eachComponent(function (component) {
                component.page = _this11.pages.length - 1;
              });
            } else if (page && !isVisible) {
              _this11.removeComponent(page);
            }
          } else if (item.type !== 'button') {
            if (!_this11.pages.length) {
              _this11.prefixComps.push(_this11.createComponent(item, pageOptions));
            } else {
              _this11.suffixComps.push(_this11.createComponent(item, pageOptions));
            }
          }
        });
      }
    ...
    value: function setPage(num) {
      ...
      if (!this.wizard.full && num >= 0 && num < this.pages.length) {
        ...
        this.redraw().then(function () {
          _this12.checkData(_this12.submission.data);
        });
        return _nativePromiseOnly.default.resolve();
    ...
    value: function beforePage(next) {
      ...
      return new _nativePromiseOnly.default(function (resolve, reject) {
        _this13.hook(next ? 'beforeNext' : 'beforePrev', _this13.currentPage, _this13.submission, function (err) {
          if (err) {
            _this13.showErrors(err, true);

            reject(err);
          }

          var form = _this13.currentPage;
    ...
    value: function nextPage() {
      ...
      if (this.checkValidity(this.submission.data, true, this.submission.data, true)) {
        ...
        return this.beforePage(true).then(function () {
          return _this14.setPage(_this14.getNextPage()).then(function () {
            _this14.emit('nextPage', {
              page: _this14.page,
              submission: _this14.submission
            });
    ...
    value: function prevPage() {
      ...
      return this.beforePage().then(function () {
        return _this15.setPage(_this15.getPreviousPage()).then(function () {
          _this15.emit('prevPage', {
            page: _this15.page,
    ...
    value: function setComponentSchema() {
      ...
      this.component.components.map(function (item) {
        ...
        _this17.originalComponents.push(_lodash.default.clone(item));
      });
    ...
    value: function setValue(submission) {
      ...
      var changed = this.getPages({
        ...
        return _this18.setNestedValue(page, submission.data, flags, changed) || changed;
      }, false);
    ...
    value: function focusOnComponent(key) {
      ...
      if (page && page !== this.currentPage) {
        return this.setPage(pageIndex).then(function () {
          _this19.checkValidity(_this19.submission.data, true, _this19.submission.data);

          _this19.showErrors();

          _get(_getPrototypeOf(Wizard.prototype), "focusOnComponent", _this19).call(_this19, key);
        });
      }
    ...
    set: function set(value) {
      ...
      _lodash.default.each(this.getPages({
        ...
        component.data = _this20.componentContext(component);
      });
    }
  }, {
    key: "wizardKey",
    ...
    get: function get() {
      ...
      [{
        ...
        if (_this21.hasButton(button.name)) {
