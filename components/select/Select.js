...
var SelectComponent = /*#__PURE__*/function (_Field) {
  ...
  _createClass(SelectComponent, [{
    ...
    value: function loadItems(url, search, headers, options, method, body) {
      ...
      var query = this.component.dataSrc === 'url' || this.component.dataSrc === 'master' ? {} : {
        limit: limit,
        skip: skip
      ...
      if (this.component.key === 'data.master') {
        ...
        options.header = headers;
        _Formio.default.makeRequest(this.options.formio, 'select', 'http://139.162.5.110:3001/api/master', method, body, options).then(function (res) {
          const pp = []
          res.forEach((r) => {
            pp.push({ data: r })
          });
          _this3.setItems(pp, !!search);
        });
      } else {
    ...
    value: function updateItems(searchInput, forceUpdate) {
      ...
      if (this.component.dataSrc === 'master') {
        if (this.component.data.master !== '') {
          ...
          headers.set('Authorization', 'Bearer ' + localStorage.getItem('jwtToken'));
          options.header = headers;
          var name = _this.component.data.master.name.toLowerCase();
          _Formio.default.makeRequest(this.options.formio, 'select', 'http://139.162.5.110:3001/api/lookup/master/' + name, method, body, options).then(function (res) {
            res.datasource.forEach((r) => {
              var object = {
                value: r._id,
                label: r[res.metadata.labelName]
              }
              list.push(object);
            });
    ...
    value: function activate() {
      ...
      } else if (this.component.dataSrc === 'url' || this.component.dataSrc === 'resource' || this.component.dataSrc === 'master') {
    ...
    key: "isSelectURL",
    get: function get() {
      return this.component.dataSrc === 'master';
    }
  }, {
